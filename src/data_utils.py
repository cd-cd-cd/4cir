...
data_path = Path("/amax/home/chendian/dataset")
...
class FashionIQDataset(Dataset):
    ...
    def __init__(self, split: str, dress_types: List[str], mode: str, preprocess: callable, **kwargs):
        ...
        for dress_type in dress_types:
            with open(data_path / 'fashionIQ' / 'captions' / f'cap.{dress_type}.{split}.json') as f:
                self.triplets.extend(json.load(f))

        # get the image names
        self.image_names: list = []
        for dress_type in dress_types:
            with open(data_path / 'fashionIQ' / 'image_splits' / f'split.{dress_type}.{split}.json') as f:
    ...
    def __getitem__(self, index):
        try:
            if self.mode == 'relative':
                ...
                if self.split == 'train':
                    reference_image_path = data_path / 'fashionIQ' / 'images' / f"{reference_name}.png"
                    reference_image = self.preprocess(PIL.Image.open(reference_image_path))
                    target_name = self.triplets[index]['target']
                    target_image_path = data_path / 'fashionIQ' / 'images' / f"{target_name}.png"
                ...
                image_path = data_path / 'fashionIQ' / 'images' / f"{image_name}.png"
...
class CIRRDataset(Dataset):
    ...
    def __init__(self, split: str, mode: str, preprocess: callable, use_sub=False, **kwargs):
        ...
        with open(data_path / 'cirr_dataset' / 'cirr' / 'captions' / f'cap.rc2.{split}.json') as f:
            self.triplets = json.load(f)
        if use_sub:
            if self.split == 'train':
                print(f"Training with sub_split of fifth. Overwriting self.triplets")
                _ = input("\nConfirm!")
                with open(data_path / 'cirr_dataset' / 'cirr' / 'captions_sub' / f'cap.rc2.{split}_fifth.json') as f:
                    self.triplets = json.load(f)

        # get a mapping from image name to relative path
        with open(data_path / 'cirr_dataset' / 'cirr' / 'image_splits' / f'split.rc2.{split}.json') as f:
    ...
    def __getitem__(self, index):
        try:
            if self.mode == 'relative':
                ...
                if self.split == 'train':
                    reference_image_path = data_path / 'cirr_dataset' / self.name_to_relpath[reference_name]
                    reference_image = self.preprocess(PIL.Image.open(reference_image_path))
                    target_hard_name = self.triplets[index]['target_hard']
                    target_image_path = data_path / 'cirr_dataset' / self.name_to_relpath[target_hard_name]
                    target_image = self.preprocess(PIL.Image.open(target_image_path))
                ...
                image_path = data_path / 'cirr_dataset' / self.name_to_relpath[image_name]
